...
const processCsvStreaming = async (file: File, map: Record<string, string>, storageKey: string, sites: Record<string, string>[] | null, listings: Record<string, string>[] | null, users: Record<string, string>[] | null) => {
  ...
  self.postMessage({
    ...
    headerDiag,
    analytics,
    metadata: { parser: 'csv-stream', rowCount: store.getRowCount(), fileName: file.name, fileSize: file.size, storageEngine: store.getEngine(), storageKey },
  });
};
  ...
  try {
    if (msg.type === 'parse-small') {
      ...
      self.postMessage({
        ...
        metadata: { parser: 'xlsx-worker', rowCount: store.getRowCount(), fileName: msg.fileName, fileSize: msg.fileSize, storageEngine: store.getEngine(), storageKey: msg.storageKey },
      });
      return;
    }
