...
export type IngestRoute = 'auto' | 'small' | 'large';
export type IngestMetadata = {
  parser: 'csv-stream' | 'xlsx-worker';
  ingestRoute: IngestRoute;
  rowCount: number;
  fileName: string;
  fileSize: number;
  storageEngine: 'opfs' | 'idb';
  storageKey: string;
  diagnostics?: {
    sourceRows: number;
    acceptedRows: number;
...
export function useFileParser() {
  ...
  const recomputeFromStore = useCallback((storageKey: string, includeTest: boolean, regionRefs: string[], initialTargetRefs?: string[], ctx?: { sites: Record<string, string>[] | null; listings: Record<string, string>[] | null; users: Record<string, string>[] | null }) => {
    if (!workerRef.current) return;
    setIsLoading(true);
    setProgress(p => p ? { ...p, stage: 'Applying filters from storage...' } : { processed: 0, total: 0, pct: 0, stage: 'Applying filters from storage...' });
    workerRef.current.postMessage({ type: 'filter-from-store', storageKey, includeTest, regionRefs, initialTargetRefs: initialTargetRefs?.length ? initialTargetRefs : undefined, sites: ctx?.sites || null, listings: ctx?.listings || null, users: ctx?.users || null });
  }, []);
